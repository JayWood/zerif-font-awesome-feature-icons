				...
				if ( ! empty( $zerif_ourfocus_custom_media_id ) && ! empty( $zerif_ourfocus_custom_media_id[0] ) ) {
					...
					<div class="service-icon">
						...
						<?php if ( ! empty( $instance['link'] ) ) { ?>
							...
							<a href="<?php echo esc_url( $instance['link'] ); ?>"><i class="pixeden" style="background:url(<?php echo esc_url( $zerif_ourfocus_custom_media_id[0] ); ?>) no-repeat center;width:100%; height:100%;"></i>
								<!-- FOCUS ICON--></a>
...
//		return $output;
	...
	function form( $instance ) {
		...
		<p class="zlfa-feature-icons">
			...
			<span class="description"><?php _e( 'Only works if image is empty.', 'zerif-lite' ); ?></span>
		</p>
		<p>
