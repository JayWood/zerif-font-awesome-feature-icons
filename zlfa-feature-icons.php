<?php
/**
 * Plugin Name: Zerif Light Font-Awesome Feature Icons
 * Plugin URI:  http://plugish.com
 * Description: A plugin which adds the ability for users to use font-awesome icons instead of uploading images in the customizer.
 * Version:     0.1.0
 * Author:      JayWood
 * Author URI:  http://plugish.com
 * Donate link: http://plugish.com
 * License:     GPLv2
 * Text Domain: zlfa-feature-icons
...
/**
 * Copyright (c) 2016 JayWood (email : jjwood2004@gmail.com)
 *
 ...
 * @return void
 */
function zlfa_feature_icons_autoload_classes( $class_name ) {
	if ( 0 !== strpos( $class_name, 'ZLFAFI_' ) ) {
		return;
	}

	$filename = strtolower( str_replace(
		'_', '-',
		substr( $class_name, strlen( 'ZLFAFI_' ) )
	) );

	ZLFA_Feature_Icons::include_file( $filename );
}
spl_autoload_register( 'zlfa_feature_icons_autoload_classes' );

/**
 * Main initiation class
 *
 * @property string version
 * @property string basenmae
 * @property string url
 * @property string path
 * @property array icons
 * @since  NEXT
 */
final class ZLFA_Feature_Icons {
	 ...
	 *
	 * @var ZLFA_Feature_Icons
	 * @since  NEXT
	 */
	protected static $single_instance = null;

	/**
	 * Instance of ZLF-AFI_Widgettestimonials
	 *
	 * @since NEXT
	 * @var ZLF-AFI_Widgettestimonials
	 */
	protected $widgettestimonials;

	/**
	 * A list of fontawesome icons
	 * @var array
	 */
	protected $icons;

	/**
	 * Instance of ZLF-AFI_Widget_Testimonails
	 *
	 * @since NEXT
	 * @var ZLF-AFI_Widget_Testimonails
	 */
	protected $widget_testimonails;

	/**
	 * Creates or returns an instance of this class.
	 *
	 * @since  NEXT
	 * @return ZLFA_Feature_Icons A single instance of this class.
	 */
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'select2', $this->url( "assets/select2-4.0.3/dist/css/select2{$min}.css" ), array(), '4.0.3' );
		wp_register_style( 'zlfa', $this->url( "assets/font-awesome-4.6.3/css/font-awesome{$min}.css" ), array(), '4.6.3' );

		wp_enqueue_style( 'zlfa-feature-icons', $this->url( "assets/zlfa-feature-icons.css" ), array( 'select2', 'zlfa' ), self::VERSION );

		wp_register_script( 'select2', $this->url( "assets/select2-4.0.3/dist/js/select2.full{$min}.js", array( 'jquery' ), '4.0.3' ), 1 );
		wp_register_script( 'zlfa-feature-icons', $this->url( "assets/zlfa-feature-icons.js" ), array( 'select2' ), self::VERSION, true );
		wp_enqueue_script( 'zlfa-feature-icons' );
	...
	public function widgets_init() {
		unregister_widget( 'zerif_testimonial_widget' );
		unregister_widget( 'zerif_ourfocus' );

		register_widget( 'ZLFAFI_Widget_Testimonials' );
		register_widget( 'ZLFAFI_Widget_Our_Focus' );
	}
...
/**
 * Grab the ZLFA_Feature_Icons object and return it.
 * Wrapper for ZLFA_Feature_Icons::get_instance()
 *
 * @since  NEXT
 * @return ZLFA_Feature_Icons  Singleton instance of plugin class.
 */
function zlfa_feature_icons() {
	return ZLFA_Feature_Icons::get_instance();
}

// Kick it off.
add_action( 'after_setup_theme', array( zlfa_feature_icons(), 'hooks' ) );

register_activation_hook( __FILE__, array( zlfa_feature_icons(), '_activate' ) );
register_deactivation_hook( __FILE__, array( zlfa_feature_icons(), '_deactivate' ) );
